...
class ProjectionLayer(nn.Module):
    def __init__(self, projection_layer_type="mlp", **kwargs):
        super().__init__()
        assert projection_layer_type in ALL_PROJECTION_LAYER_TYPES, (
            f"Unsupported projection layer type: {projection_layer_type}. Must be one of {ALL_PROJECTION_LAYER_TYPES}"
        )
        # self.kwargs = kwargs
...
class DenoiserREPA(nn.Module):
    ...
    def sample(self):
        ...
        grid = make_grid(samples, nrow=int(bsz**0.5), padding=0)
        plt.imshow(grid.permute(1, 2, 0).cpu().numpy())
        # make the borders as small as possible
    ...
    def generate(self, labels, accelerator=None):
        ...
        timesteps = (
            torch.linspace(
                0.0, 1.0, self.steps + 1, device=accelerator.device if accelerator is not None else self.device
            )
            .view(-1, *([1] * z.ndim))
            .expand(-1, bsz, -1, -1, -1)
        )
    ...
    def train_model(self, train_loader, verbose=True):
        ...
        """
        appendix = (
            self.args.enc_ckpt_path.split("/")[-1].replace(".pth", "")
            if self.args.enc_ckpt_path is not None
            else self.args.enc_type
        )
        accelerator = accelerate.Accelerator(
            log_with="wandb", gradient_accumulation_steps=self.args.gradient_accumulation_steps
        )
        if not self.no_wandb:
            accelerator.init_trackers(
                "iREPA",
                config=self.args,
                init_kwargs={"wandb": {"name": f"{self.args.vae}_{appendix}_{self.args.model}_{self.dataset}"}},
            )
        ...
        if self.args.snapshot == 1:
            next_save = 50_000  # save every 50k steps if snapshot is 1
        ...
        for epoch in epoch_bar:
                    ...
                    with torch.no_grad():
                        ...
                        with accelerator.autocast():
                            for encoder in self.encoders:
                                ...
                                # if enc_type contains sam, we need to break the batch into smaller batches
                                if "sam" in self.args.enc_type.lower():
                                    batch_size = raw_image_.shape[0]
                                    small_batch_size = batch_size // 4
                                    features_list = []
                                    for i in range(0, batch_size, small_batch_size):
                                        interval_max = min(batch_size, i + small_batch_size)
                                        raw_image_batch = raw_image_[i:interval_max]
                                        features_batch = encoder.forward_features(raw_image_batch)
                                        features_list.append(features_batch)
                                    # concatenate features
                                    features = {}
                                    for key in features_list[0].keys():
                                        tensors = [f[key] for f in features_list if f[key] is not None]
                                        features[key] = torch.cat(tensors, dim=0) if tensors else None
                                else:
            ...
            total_steps_cnt += int(len(train_loader) // accelerator.gradient_accumulation_steps)
            ...
            if (epoch + 1) % self.snapshot == 0:
                ...
                accelerator.save(
                    ema_to_save.state_dict(),
                    os.path.join(
                        models_dir,
                        "iREPA",
                        f"{self.args.vae}__{appendix}_{self.args.model.replace('/', '_')}_{self.dataset}_epoch{epoch + 1}.pt",
                    ),
                )

            elif self.args.snapshot == 1 and total_steps_cnt >= next_save:
                ema_to_save = accelerator.unwrap_model(self.ema)
                accelerator.save(
                    ema_to_save.state_dict(),
                    os.path.join(
                        models_dir,
                        "iREPA",
                        f"{self.args.vae}__{appendix}_{self.args.model.replace('/', '_')}_{self.dataset}_step{next_save}.pt",
                    ),
                )
                next_save += 50_000
            ...
            if epoch == 0 or ((epoch + 1) % self.sample_and_save_freq == 0):
                ...
                with torch.no_grad():
                    sample_labels = (
                        (torch.arange(0, 16, device=accelerator.device) % self.num_classes)
                        if self.num_classes > 0
                        else torch.zeros(16, device=accelerator.device, dtype=torch.long)
                    )
    ...
    def inpaint(self, dataloader):
        ...
        enc_imgs = self.encode(imgs.to(device))

        inpaints = self.inpaint_image(
            enc_imgs, torch.zeros(enc_imgs.size(0), dtype=torch.long, device=device), accelerator=accelerator
        )

        masked_imgs = imgs.clone()
        h, w = imgs.size(2), imgs.size(3)
        masked_imgs[:, :, h // 4 : h * 3 // 4, w // 4 : w * 3 // 4] = -1.0

        masked_imgs = masked_imgs * 0.5 + 0.5
        masked_imgs = torch.clamp(masked_imgs, 0.0, 1.0).float()
        inpaints = inpaints * 0.5 + 0.5
        inpaints = torch.clamp(inpaints, 0.0, 1.0).float().cpu()

        # concatenate the masked images and inpainted images into a grid of nrow = len(imgs)
        imgs = torch.cat([masked_imgs, inpaints], dim=0)
        grid = make_grid(imgs, nrow=imgs.size(0) // 2, padding=0)
        plt.imshow(grid.permute(1, 2, 0).cpu().numpy())
        plt.axis("off")
        # make the borders as small as possible
        plt.subplots_adjust(left=0, right=1, top=1, bottom=0)
    ...
    @torch.no_grad()
    def inpaint_image(self, images, label, accelerator=None):
        bsz = images.size(0)
        device = accelerator.device if accelerator is not None else self.device
        ...
        mask[:, :, h // 4 : h * 3 // 4, w // 4 : w * 3 // 4] = 1.0

        # 2. CREATE A SOFT MASK (The "Seam Killer")
        # A soft transition (3-5 pixels) allows the VAE to blend the regions.
        # We use a simple Gaussian blur on the binary mask.
        kernel_size = 7
        sigma = 2.0
        soft_mask = self._gaussian_blur(mask, kernel_size, sigma)
        inverted_soft_mask = 1.0 - soft_mask

        # 3. Initialize noise
        z = torch.randn(bsz, self.channels, self.img_size, self.img_size, device=device)
        context_noise = torch.randn_like(images)
        ...
        for i in tqdm(range(self.steps - 1), desc="Sampling"):
            t = timesteps[i]
            t_next = timesteps[i + 1]

            with accelerator.autocast():
                # Get the model's prediction for the next step
                z_next = stepper(z, t, t_next, label)

                # Calculate the "known" part at t_next
                # Flow Matching linear path: x_t = t*x_1 + (1-t)*x_0
                original_part = images * t_next + context_noise * (1.0 - t_next)

                # 4. SOFT BLEND
                # Instead of z = z*mask + original_part*inv_mask
                z = (z_next * soft_mask) + (original_part * inverted_soft_mask)

        # Final Step: Clean transition to pixel space
        with accelerator.autocast():
            z = self._euler_step(z, timesteps[-2], timesteps[-1], label)
            # Final blend with the actual ground truth (t=1.0)
            z = (z * soft_mask) + (images * inverted_soft_mask)

            if self.vae is not None:
                z = self.decode(z)
        return z

    def _gaussian_blur(self, x, k, s):
        """Helper to smooth the mask edges in latent space"""
        import torchvision.transforms.functional as TF

        return TF.gaussian_blur(x, kernel_size=[k, k], sigma=[s, s])

    @torch.no_grad()
    def outpaint(self, dataloader):
        """
        ...
        mask[:, :, h // 4 : h * 3 // 4, w // 4 : w * 3 // 4] = 1.0
        inverted_mask = (1.0 - mask) * -1.0
        masked_imgs = masked_imgs * mask + inverted_mask

        masked_imgs = masked_imgs * 0.5 + 0.5
        masked_imgs = torch.clamp(masked_imgs, 0.0, 1.0).float()
        outpaints = outpaints * 0.5 + 0.5
        outpaints = torch.clamp(outpaints, 0.0, 1.0).float().cpu()

        # concatenate the masked images and outpainted images into a grid of nrow = len(imgs)
        imgs = torch.cat([masked_imgs, outpaints], dim=0)
        grid = make_grid(imgs, nrow=imgs.size(0) // 2, padding=0)
        plt.imshow(grid.permute(1, 2, 0).cpu().numpy())
        plt.axis("off")
        # make the borders as small as possible
    ...
    @torch.no_grad()
    def outpaint_image(self, images, label, accelerator=None):
        """
        Outpaint the input image (preserve center, generate borders)
        """
        bsz = images.size(0)
        device = accelerator.device if accelerator is not None else self.device

        # 1. Create the binary mask (1.0 for the area to be GENERATED)
        # We start with ones and set the center (known part) to zero.
        mask = torch.ones_like(images)
        h, w = images.size(2), images.size(3)
        # The center is what we already have (the context)
        mask[:, :, h // 4 : h * 3 // 4, w // 4 : w * 3 // 4] = 0.0

        # 2. CREATE THE SOFT TRANSITION
        # We blur the mask so the transition from 'known center' to 'generated edge' is smooth.
        # A larger kernel (e.g., 11 or 15) usually works better for outpainting.
        soft_mask = self._gaussian_blur(mask, k=11, s=3.0)
        inverted_soft_mask = 1.0 - soft_mask

        # 3. Initialize noise and context noise
        z = torch.randn(bsz, self.channels, self.img_size, self.img_size, device=device)
        context_noise = torch.randn_like(images)
        ...
        # 4. ODE Sampling Loop
        for i in tqdm(range(self.steps - 1), desc="Outpainting Steps"):
            t = timesteps[i]
            t_next = timesteps[i + 1]

            with accelerator.autocast():
                # Predict the full image (center + edges)
                z_next = stepper(z, t, t_next, label)

                # Construct the noised version of our known center at t_next
                # Flow Matching: x_t = x_1 * t + x_0 * (1 - t)
                original_center = images * t_next + context_noise * (1.0 - t_next)

                # 5. BLEND
                # z_next * soft_mask         -> Keep predicted EDGES
                # original_center * inv_mask -> Keep noised CENTER
                z = (z_next * soft_mask) + (original_center * inverted_soft_mask)

        # Final Step: Clean transition
        with accelerator.autocast():
            z = self._euler_step(z, timesteps[-2], timesteps[-1], label)

            # Final stitch with the clean original latents (t=1.0)
            z = (z * soft_mask) + (images * inverted_soft_mask)
    ...
    def fid_sample(self):
        ...
        labels = (
            torch.arange(0, 50000, device=device) % self.num_classes
            if self.num_classes > 0
            else torch.zeros(50000, device=device, dtype=torch.long)
        )
        ...
        for i in tqdm(range(0, 50000, bsz), desc="FID Sampling", leave=False):
            ...
            for j in range(current_bsz):
                idx = i + j
                plt.imsave(os.path.join(fid_dir, f"{idx:05d}.png"), samples[j].permute(1, 2, 0).cpu().numpy())
