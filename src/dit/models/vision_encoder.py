...
GASTRONET_MEAN = [0.64041256, 0.36125767, 0.31330117]
GASTRONET_STD = [0.18983584, 0.15554344, 0.14093774]
...
def make_dinov3_transform(resize_size: int = 224, gastro: bool = False):
    to_tensor = transforms.Lambda(lambda x: x / 255.0)
...
def load_dinov3(model_name, checkpoint_path: Optional[str] = None):
    ...
    model = torch.hub.load(
        ...
        model_name,
        source="local",
        untie_global_and_local_cls_norm=False, #True if checkpoint_path is not None else False,
        weights=checkpoint_path
        if checkpoint_path is not None
        else os.path.join(PRETRAINED_DIR, f"{model_name}_pretrain_lvd1689m-{SHA_CHECKSUM[model_name]}.pth"),
    )
    return model
...
class DINOEncoder(VisionEncoder):
    ...
    def forward_features(self, x: torch.Tensor) -> Dict[str, Optional[torch.Tensor]]:
        ...
        return {"x_norm_clstoken": out[:, 0], "x_norm_patchtokens": out[:, 1:]}
...
class DINOv2Encoder(VisionEncoder):
    ...
    def forward_features(self, x: torch.Tensor) -> Dict[str, Optional[torch.Tensor]]:
        ...
        out = self.model.forward_features(x)
        return {"x_norm_clstoken": out.get("x_norm_clstoken"), "x_norm_patchtokens": out.get("x_norm_patchtokens")}
...
class DINOv2MixedEncoder(DINOv2Encoder):
    ...
    def forward_features(self, x: torch.Tensor) -> Dict[str, Optional[torch.Tensor]]:
        ...
        return {"x_norm_clstoken": cls_token, "x_norm_patchtokens": mixed_patch_tokens}
...
class DINOv3Encoder(VisionEncoder):
    ...
    def preprocess(self, x: torch.Tensor) -> torch.Tensor:
        transform_func = make_dinov3_transform(resize_size=self.resolution, gastro=self.gastro)
    ...
    def forward_features(self, x):
        ...
        if hidden_states.shape[-1] != self.resolution // 16:
            hidden_states = torch.nn.functional.interpolate(
                hidden_states, size=(self.resolution // 16, self.resolution // 16), mode="bilinear", align_corners=False
            ).permute(0, 2, 3, 1)
        return {
...
class SAM2LogitEncoder(VisionEncoder):
    ...
    def forward_features(self, x):
        ...
        dense_features = torch.stack([torch.cat(all_results[i], dim=0) for i in range(B)], dim=0)

        # Downsample if needed
        dense_features = torch.nn.functional.interpolate(
            dense_features, size=(self.target_resolution, self.target_resolution), mode="bilinear", align_corners=False
        )
        return {
            "x_norm_clstoken": None,
...
class PEEncoder(VisionEncoder):
    ...
    def load_model(self):
        ...
        if self.encoder_type == "pe":
            ...
            config_map = {"b": "PE-Spatial-B16-512", "l": "PE-Spatial-L14-448", "g": "PE-Spatial-G14-448"}
        elif self.encoder_type == "langpe":
            config_map = {"l": "PE-Lang-L14-448", "g": "PE-Lang-G14-448"}
        else:
    ...
    def preprocess(self, x: torch.Tensor) -> torch.Tensor:
        x = x / 255.0
        x = torch.nn.functional.interpolate(x, self.patch_size * (self.resolution // 16), mode="bilinear")
...
def create_encoder(
    ...
    if len(parts) != 3:
        raise ValueError(
            f"Invalid encoder string format: {encoder_string}. Expected format: encoder_type-architecture-model_config"
        )

    encoder_type, architecture, model_config = parts

    if encoder_type not in ENCODER_REGISTRY:
        raise ValueError(f"Unknown encoder type: {encoder_type}. Available types: {list(ENCODER_REGISTRY.keys())}")

    encoder_class = ENCODER_REGISTRY[encoder_type]
    encoder = encoder_class(encoder_type, architecture, model_config, device, resolution, accelerator)
    if checkpoint_path is None:
        encoder.load_model()
    else:
...
@torch.no_grad()
def load_encoders(
    enc_type: str, device: torch.device, resolution: int = 256, accelerator=None, checkpoint_path: Optional[str] = None
) -> List[VisionEncoder]:
    """
