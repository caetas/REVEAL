...
class RF(nn.Module):
    ...
    def __init__(self, args, img_size, channels, ln=True):
        ...
        if args.vae == "SD2":
            self.vae =  AutoencoderKL.from_pretrained(f"stabilityai/stable-diffusion-2-1-base", subfolder='vae')
            self.channels = 4
            ...
            self.vae =  AutoencoderKL.from_pretrained(f"black-forest-labs/FLUX.2-dev", subfolder='vae')
            self.channels = 32
            self.img_size = img_size // 8
    ...
    def train_model(self, train_loader, verbose=True):
        ...
        if not self.no_wandb:
            accelerate.init_trackers("DiT", config=self.args, init_kwargs={"wandb": {"name": f"{self.args.model}_{self.dataset}"}})
