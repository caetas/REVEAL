...
def get_optimized_compile_mode():
    ...
    # Heuristic: max-autotune requires high parallelism
    # You can adjust this threshold (e.g., 40 or 80) based on your needs
    if sm_count >= 80:
        return "max-autotune"
    elif sm_count >= 20:
...
class Denoiser(nn.Module):
    ...
    def sample(self):
        ...
        samples = self.generate(labels, accelerator=accelerator)
    ...
    def generate(self, labels, accelerator=None):
        bsz = labels.size(0)
        z = self.noise_scale * torch.randn(bsz, 3, self.img_size, self.img_size, device=accelerator.device if accelerator is not None else self.device)
    ...
    def _forward_sample(self, z, t, labels):
        ...
        if self.num_classes == 0 or self.cfg_scale == 0:
            return v_uncond
        ...
        for epoch in tqdm(range(self.n_epochs), desc="Training Epochs"):
            ...
            if (epoch + 1) % self.args.sample_and_save_freq == 0 or epoch == 0:
                ...
                with torch.no_grad():
                    ...
                    samples = self.generate(sample_labels, accelerator=accelerator)
    ...
    def fid_sample(self):
        ...
        epoch = self.args.checkpoint.split('_')[-2].replace('.pt', '').replace('epoch', '')
        ...
        fid_dir = f"../../fid_samples/{self.dataset}/{self.args.model.replace('/', '_')}_steps{self.steps}_solver{self.method}_cfg{self.cfg_scale}_epoch{epoch}"
